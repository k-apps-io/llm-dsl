...
chat
  ...
  .prompt({
    message:
      'send me 3 progamming languages as a JSON array e.g ```json ["lang1", "lang2", "langN"]```',
...
chat
  .rule({
    name: "Bob Uecker",
    requirement:
      "respond in the style of Bob Uecker, the Famous Radio Broadcaster and not so Fammous American League Baseball Player",
  })
  .prompt({
...
chat
  ...
  .prompt(({ locals, chat: $chat }) => {
    if (locals.isChill) {
      ...
      return {
        message: `nope, wrong... ${toCodeBlock("json", {
          todos: $chat.locals.tasks,
        })}`,
      };
    }
  })
...
chat
  .prompt({
    message:
      "please generate a JSON Array of golf clubs I could hit out of the ruff. This JSON array must structured as ```typescript {\noptions: { club: string; yardage: number; }[]\n}``` and be provided as a JSON code block",
    response_format: { type: "json_object" },
  })
  .expect(
    json(),
...
chat
  ...
  .prompt({
    message:
      "I have 120y to the hole and I want to carry it 115y, which club should I use?",
  })
  .prompt({
...
chat
  ...
  .prompt({
    message:
      "generate a list of cities implementing the ```typescript interface { cities: string[] }``` as a json code block",
    response_format: { type: "json_object" },
  })
  .expect(
    json(),
  ...
  .prompt({
    message:
      "consider the RPG table top game Sprawl, create a new mission in this city that involves our target",
  })
  .response(({ response }) => {
