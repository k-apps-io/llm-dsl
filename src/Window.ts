 ...
 */
export const latest = ( { n }: LatestWindowOptions ): LLM.Window => {
  const reduce: LLM.Window = ( { messages }: LLM.WindowOptions ) => {
 ...
 */
export const main: LLM.Window = ( { chat, messages, tokenLimit } ) => {
  const _messages: LLM.Chat<any>[ "messages" ] = messages
    // reduce to the latest keys
    .reduce( ( prev, curr, index ) => {
      if ( curr.key !== undefined ) {
        const prevIndex = prev.findIndex( ( p, i ) => p.key === curr.key && p.id !== curr.id && i < index );
        // a message id that shares the key with the current message
        // if defined this message will be excluded.
        let targetId: string | undefined = undefined;
        if ( prevIndex > -1 ) {
          // previous message in the history shares the current key
          targetId = prev[ prevIndex ].id;
        }
        // include all messages in prev that are not the target
        if ( targetId ) prev = prev.filter( p => p.id !== targetId );
      }
      return prev;
    }, messages as LLM.Chat<any>[ "messages" ] )
    ;
  ...
  const _window = _messages
    ...
    .filter( m => m.message.visibility !== LLM.Visibility.EXCLUDE )
    ...
    .sort( ( a, b ) => {
      if ( a.message.visibility === LLM.Visibility.REQUIRED && b.message.visibility !== LLM.Visibility.REQUIRED ) return -1;
      if ( a.message.visibility !== LLM.Visibility.REQUIRED && b.message.visibility === LLM.Visibility.REQUIRED ) return 1;
    ...
    .reduce( ( prev, curr ) => {
      ...
      return prev;
    }, [] as { index: number, message: LLM.Message.Message<any, any, any, any>; }[] );
